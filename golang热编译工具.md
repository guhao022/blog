...
之前用beego写api，感觉bee命令很好用，后来抛弃框架了，bee命令也不合适了，写程序编译每次都用 go run 感觉有点麻烦了，所以决定自己写一个热编译工具。

热编译工具一般是通过检测文件变化，并触发相应编译命令。
现在已经有一些好用的热编译工具了，比如之前说的 [bee](https://github.com/beego/bee) 工具，[fswatch](https://github.com/codeskyblue/fswatch) 都还是挺不错的，可以研究一下他们的代码，如果想练手就自己写一个吧  o(-"-)o 

用到的外部包只有一个 [fsnotify](https://github.com/go-fsnotify/fsnotify), fsnotify 是一个文件夹监控应用，在[example_test.go](https://github.com/go-fsnotify/fsnotify/blob/master/example_test.go)中给的是最简单的实际应用
...
代码比较简单，基本原理就是利用 fsnotify 监视文件变化，然后利用 os/exec 包去 编译、启动、终止进程
